...
exports.chatroomGetAll = (req, res) => {
    ...
    pool.query(`SELECT chatrooms.chat_id, name, icon, content FROM chatrooms JOIN members ON chatrooms.chat_id=members.chat_id JOIN messages ON chatrooms.chat_id=messages.chat_id WHERE members.s_id=${s_id} ORDER BY sent LIMIT 1`, (err, results) => {
        if (err) throw err;
        else {
            if (results.length === 0) {
                res.json({code: 401});
